...
app.get('/api/mcp-status', async (req, res) => {
    try {
        ...
        const [configData, runtimeData, cliData] = await Promise.allSettled([
            ...
            // 方法2: 运行时检测
            (async () => {
                console.log('🔍 方法2: 运行时检测');
                return await detectMcpRuntime();
            })(),
            
            // 方法3: Claude CLI命令检测
            (async () => {
                console.log('🔍 方法3: Claude CLI命令检测');
        ...
        console.log('🔍 检测结果汇总:');
        console.log('📋 配置文件检测:', configResult.success ? '成功' : '失败', configResult.servers || []);
        console.log('📋 CLI命令检测:', cliResult.success ? '成功' : '失败', cliResult.servers || []);
        console.log('📋 运行时检测:', runtimeResult.hasMcpSupport ? '成功' : '失败');
        console.log('📋 最终合并结果:', finalMcpServers);
